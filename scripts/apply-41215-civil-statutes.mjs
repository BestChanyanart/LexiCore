...
const statutesPath = path.join(
  repoRoot,
  "content/courses/41215/source-materials/civil-law-sections-short-title.csv",
);
const questionsPath = path.join(
  repoRoot,
  "content/courses/41215/question-bank/writing-question-table-41215.csv",
);
...
function sectionRefs(value) {
  ...
  return String(value || "")
    .split(",")
    .map((raw) => {
      const label = raw.trim().replace(/^มาตรา\s*/u, "");
...
function splitStatuteParagraphs(content) {
  return String(content || "")
    .replace(/\s+(วรรคหนึ่ง)/gu, "\n$1")
    .replace(/\s+(วรรคสอง)/gu, "\n$1")
    .replace(/\s+(วรรคสาม)/gu, "\n$1")
    .replace(/\s+(วรรคสี่)/gu, "\n$1")
    .replace(/\s+(ถ้าการกระทำ)/gu, "\n$1")
    .replace(/\s+(ในกรณี)/gu, "\n$1")
    .replace(/\s+(แต่ถ้า)/gu, "\n$1")
    .split(/\n+/)
